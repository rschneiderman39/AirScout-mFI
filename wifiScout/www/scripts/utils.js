...
(function() {
  ...
  app.utils.deepCopy = function(object) {
    return JSON.parse(JSON.stringify(object));
  };

  app.utils.destroy = function(obj) {
    for (var key in obj) {
  ...
  app.utils.hiddenSSIDSort = function(AP) {
    if (AP.SSID.charAt(0) === '<') {
  ...
  app.utils.isView = function(view) {
    return app.strings.viewTitles[view] !== undefined;
  };

  app.utils.setAlpha = function(color, alpha) {
    var attrs = color.split(',');
    attrs[3] = alpha.toString() + ')';
    return attrs.join(",");
  };

  app.utils.spanLen = function(span) {
  ...
  app.utils.windowCheck = function() {
      mobileDevice = false;
